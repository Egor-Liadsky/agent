...
- [ ] 1.1 Убрать `ContextStrategy::MemoryLayers` из `ALL`, `label`,
      ...
      проходит.
- [ ] 1.2 Убрать `ChatSettings.memory_short_term_tail`; добавить
      ...
      игнорируется serde).
- [ ] 1.3 Исправить прямые конструкторы `ChatSettings { .. }` в
...
- [ ] 1.4 Запушить `agent-cli` в `main`; в `agent-sever` временно убрать
...
- [ ] 2.1 В `src/memory.rs` заменить `MemoryLayersImpl`/`assemble` (реализацию
...
- [ ] 2.2 В `src/context.rs` удалить `MemoryLayersImpl` и ветку
...
- [ ] 2.3 Убрать `ContextDto::for_memory_layers` (`src/dto.rs`); поля памяти
...
- [ ] 2.4 Тест: при выключенной памяти (`memory_layers_enabled: false` или
...
- [ ] 3.1 В `src/app.rs` заменить условие запуска
...
- [ ] 3.2 В `src/config.rs` добавить `AGENTD_MEMORY_LAYERS_ENABLED` (bool,
      ...
      значение).
- [ ] 3.3 Добавить `crate::memory::effective_layers_enabled(state, settings)
      -> bool` (настройка чата поверх операторского умолчания, по образцу
      `effective_router_enabled`); тест переопределения умолчания
      настройкой чата.
- [ ] 3.4 Убедиться, что доступность слоистой памяти не зависит от
...
- [ ] 4.1 Параметризованный (или явно по одному на стратегию) интеграционный
...
- [ ] 4.2 Тест сценария `facts` + память: факт, сохранённый ранее, и запись
      ...
      разделе.
- [ ] 4.3 Обновить существующие тесты `add-memory-layers`, писавшие
...
- [ ] 5.1 В `SettingsEditor`/`FormatField` (`tui.rs`) убрать «Слои памяти»
...
- [ ] 5.2 Поля `MemoryRouterEnabled`/`MemoryWorkingMaxEntries`/`MemoryLongTermMaxEntries`
...
- [ ] 6.1 Обновить `agent-sever/README.md`: раздел «Стратегии контекста»
...
- [ ] 6.2 Обновить `agent-cli/README.md`: раздел «Контекст» параметров чата
      и раздел «Память чата» — переключатель отдельно от перебора
      стратегий, без поля «Хвост краткосрочной памяти».

## 7. Финальная проверка

- [ ] 7.1 Итоговый прогон `cargo test` в `agent-cli` и в `agent-sever`, а
      также `cargo test --locked` в `agent-sever` без `[patch]` — все
      проходят.
- [ ] 7.2 Убедиться, что `docs/context-strategies-comparison.md` и
